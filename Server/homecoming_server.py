...
# **시군구 코드는 자료에 물어서 얻는다.**
#
# 경유정류장 API(`BusRoutespecificStopInformation`)는 `sgg_cd` 를 요구하는데, 그 값을
# **이 계열 안에서는** 좌표나 노선번호에서 얻지 못했다(2026-08-21 확인). 노선표의
# `sgg_cd` 는 경기 4,671개 노선 전부 `41000`(도 단위 자리표시자)이고, 시군구 목록
# API(`BusLcInfoInqireService`)는 이 키에 권한이 없다(403).
#
# **2026-08-27 — 경기도는 이 길을 안 써도 된다.** GBIS 의
# `getBusRouteStationListv2` 는 `routeId` 하나로 경유정류소를 주고 `sgg_cd` 가
# 아예 필요 없다(`gbis_route_stops`). 아래 표는 서울 경유정류장을 다룰 때만 쓴다.
#
...
def arrival_recently_measured(route_no, lat, lon, at):
    ...
    **사람이 새로고침을 톡톡 누르면 그대로 다 나간다.** 2026-08-27 에 1초
    간격으로 네 번 나갔다. 버튼에 중복 방지가 있지만(`refreshing`) 응답이 오면
    풀리는데 왕복이 1초 미만이라, 1초에 한 번씩 누르면 매번 통과한다.

    누르는 사람을 탓할 일이 아니다 — 값이 궁금해서 누르는 것이고, 1초 만에
    다시 물어도 **같은 값이 온다.** 자원을 가진 쪽에서 막는 것이 맞다.
    """
