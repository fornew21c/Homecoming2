...
# **시군구 코드는 알아낼 방법이 없어서 목록으로 들고 있는다.**
#
# 경유정류장 API 는 `sgg_cd` 를 요구하는데, 그 값을 좌표나 노선번호에서 얻는 길이
# 없다. 노선표의 `sgg_cd` 는 4,671개 노선 전부 `41000`(도 단위 자리표시자)이고,
# 시군구 목록 API(`BusLcInfoInqireService`)는 이 키에 권한이 없다(403).
# 코드 공간을 훑는 것도 막힌다 — 동시 요청이 조용히 실패해서 결과를 믿을 수 없다.
#
# 그래서 **실제로 자료가 오는 것을 확인한 코드만** 여기 적는다. 확인 방법은
# 그 코드로 이 API 를 불러 정류장이 오는지 보는 것이다(2026-08-20 확인).
# 다른 지역이 필요해지면 같은 방법으로 확인해서 더한다. 목록에 없는 지역은
# 빈 결과가 되고, 앱은 지금처럼 자동차 경로로 그린다 — 조용히 틀리지는 않는다.
BUS_SGG = {
    "41": ["41285", "41281", "41287"],   # 고양시 일산동구 · 덕양구 · 일산서구
}
