...
struct HomecomingJourneyMap: View {
    ...
    private func content(traveler: CLLocationCoordinate2D, home: CLLocationCoordinate2D?) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // 큰 화면에서는 아직 좁았다. 그래서 보이는 높이의 비율로 잡는다.
            // `containerRelativeFrame` 이 재는 것은 스크롤 컨테이너의 보이는
            // 높이다(iOS 17+).
            //
            // **46% 에서 72% 로 키웠다.** 지도를 크게 보고 싶다는 요구다.
            //
            // 그 대가를 알고 쓴다 — 이 지도는 **이미 화면 밖에서 시작한다.** 위에
            // 있는 카드가 정류장 10개짜리 노선도 때문에 450pt 를 먹어서, 지도 틀이
            // 화면 606pt 에서 시작해 942pt 에서 끝난다(iPhone 17 Pro, 화면 874pt,
            // 2026-08-25 실측). 46% 일 때 이미 68pt 가 화면 밖이었다. 키우면 그만큼
            // 더 나가고, 온전히 보려면 스크롤해야 한다.
            //
            // 상한을 620 으로 둔 이유는 그 이상은 어차피 한 화면에 안 들어와서다.
            // 하한 300 은 그대로다 — 작은 화면에서 28km 가 다시 눌리지 않게.
            .containerRelativeFrame(.vertical) { height, _ in
                min(max(height * 0.72, 300), 620)
            }
