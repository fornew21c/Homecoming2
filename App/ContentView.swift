    ...
    private var travelerStripCard: some View {
        if activity.isRunning,
           ...
           let state = activity.currentState {
            ...
            // 있는 동안만 스스로 다시 묻는다.
            //
            // **칩이 없을 때야말로 돌아야 한다.** 예전에는 `busArrivalNo` 가
            // 있을 때만 돌았다 — 승차 15분 전이 아닌 내내 공공데이터를 태우지
            // 않으려던 것인데, 그 조건이 칩을 되살릴 유일한 길을 같이 막았다.
            //
            // 2026-08-27 실주행에서 풍산역 정류장에 서 있는 동안 칩이 사라졌고
            // 돌아오지 않았다. 값이 사라지면 이 되묻기가 멈추고, 새로고침 버튼은
            // 칩 안에 있어 같이 사라진다 — **손으로도 되살릴 수 없었다.** 남은
            // 길은 heartbeat 뿐인데 서서 기다리는 동안에는 그게 2분에 한 번이다.
            //
            // **창 검사는 서버가 한다**(`handle_bus_arrival` 의 `far`). 자원을
            // 가진 쪽에서 막는 것이 맞고, 앱은 창이 어디인지 모른다 — 경로 좌표도
            // 구간 시각도 서버가 들고 있다.
            //
