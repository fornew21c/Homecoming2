...
struct HomecomingAttributes: ActivityAttributes {
    ...
    struct ContentState: Codable, Hashable {
        ...
        /// 서버는 **승차 15분 전부터만** 싣는다.
        ///
        /// **번호만 오고 시각이 없을 수 있다.** 창은 열렸는데 실시간 도착정보에
        /// 그 차가 아직 안 잡힌 시간이다 — 차가 11~17정류장 안에 들어와야 나온다
        /// (2026-08-27 실측: 999 첫차 17분 뒤가 가장 먼 값이고 둘째 차는 아예 없다).
        /// 그날 창이 18:17 에 열렸는데 값은 18:27:39 에 왔다.
        ///
        /// 그때 화면은 `999번 · 도착 정보 아직 없음` 을 그린다. **없는 것과 아직
        /// 모르는 것을 가르려는 것이다** — 둘이 똑같이 보여서 고장으로 읽혔다.
        ///
        /// (예전 주석은 `셋이 함께 오거나 함께 없다` 였다. 2026-08-28 에 깨졌다.)
        ///
        /// **없는 것이 기본이다.** 승차가 아직 멀면 번호도 안 온다. 화면은 그때
        /// 줄을 안 그린다 — 틀린 값을 그리는 것보다 안 그리는 것이 낫다.
        ///
        /// 서울 시내버스도 이제 온다(2026-08-27, TOPIS). 예전 주석은 `163번
        /// 구간에는 영영 안 온다` 였는데 그건 TAGO 만 보던 때의 이야기다.
        ///
